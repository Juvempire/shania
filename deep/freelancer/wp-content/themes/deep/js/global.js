window.deepOptions = deepOptions;
...
if (window.location.hash) {
	window.boldwayOrigHash = window.location.hash;
...
function correct_deep_mega_menu(){
	jQuery('header:not(.headerclone) .navbar-collapse li.deep_mega_menu > ul.menu-depth-1, .header_style8_left_container .navbar-collapse li.deep_mega_menu > ul.menu-depth-1, .header_style9_left_container .navbar-collapse li.deep_mega_menu > ul.menu-depth-1').each(function(){
	...
	jQuery(document).on('click', 'header:not(.headerclone) .sliderbar-menu-controller, .deep-push-sidebar .sliderbar-menu-controller', function (event) {
		if (!jQuery('.deep-push-sidebar').hasClass('opened')) {
		...
		jQuery('.deep-push-sidebar').toggleClass('opened');
		jQuery('.sliderbar-menu-controller .hamburguer-trigger-menu-icon').toggleClass('sidebar-opened');
		jQuery('html,body').toggleClass('deep-push-sidebar-opened'); 
		setTimeout(function(){
			if (!jQuery('.deep-push-sidebar').hasClass('opened')) {
...
jQuery(function(){
	...
	jQuery('.navbar-nav li').not('.menu-item-depth-1').removeClass('deep_mega_hide_title').removeClass('deep_mega_hide_link');
	jQuery('.navbar-nav li.deep_mega_hide_link > a').attr('href','#');
				...
				if (jQuery(this).children('.sub-arrow').length){
					if (!jQuery(this).children('.sub-arrow').is(':hover')){
						jQuery('html, body').animate({
							...
							easing: 'easeInOutQuart',
							complete: function(){
										 if (window.deepOptions.deep_update_section_titles == 'on' && jQuery(target).data('sectionTitle')){
	...
	if (jQuery('header').length && !jQuery('body.style9').length){
		...
		if (theheader.hasClass('style8') || theheader.hasClass('style9')){
			window.deepOptions.deep_header_after_scroll = "no";
			window.deepOptions.deep_header_after_scroll_type2 = "no";
			window.deepOptions.deep_header_shrink = "no";
		}
		
		/* hideonstart stuff */
		if (theheader.hasClass('hide-on-start') || window.deepOptions.deep_header_after_scroll_type2 == 'yes') {
			window.deepOptions.deep_header_after_scroll = 'yes';
			window.deepOptions.deep_header_shrink = 'no';
		}
		...
		if (window.deepOptions.deep_header_after_scroll_type2 != 'no') jQuery('.headerclone').removeClass('header_after_scroll');
		...
		if (window.deepOptions.deep_header_shrink == 'yes' || window.deepOptions.deep_header_after_scroll == 'yes'){
			
			var header_after_scroll_class = 'header_after_scroll';
			if (window.deepOptions.deep_header_after_scroll_type2 == 'yes'){
				...
				jQuery('body').waypoint({
					handler: function(event, direction) {
						...
						correct_deep_mega_menu();
					},
					offset: waypoint_offset+"px"
				});
			}
			
			if (window.deepOptions.deep_header_after_scroll_type2 == 'no') jQuery(document).scrollTop(0);
			var pagePos = window.pageYOffset || window.scrollTop;
			if ( pagePos > Math.floor(jQuery(window).height()/changeFactor) ) theheader.addClass('header_force_opacity');
			  theheader.addClass(header_after_scroll_class);
			  if (jQuery(window).width() > 767) topbar.css('margin-top','');
			...
			if (window.deepOptions.deep_header_after_scroll_type2 == 'no') jQuery(document).scrollTop(initialScroll);
			if (initialScroll < 301 && !theheader.hasClass('hide-on-start') && window.deepOptions.deep_header_after_scroll_type2 == 'no') jQuery('header').removeClass(header_after_scroll_class);
	...
	if (jQuery('section.page_content').length > 1){
		...
		sections.waypoint({
			handler: function(event, direction) {
				...
				if (window.deepOptions.deep_update_section_titles == 'on' && jQuery('body > #main > section').length > 2){
	...
	//ajax search
	var form = jQuery('.deep_search_input.search_input');
	var search_ajaxing = null;
	jQuery('header:not(.headerclone) .search_trigger, header:not(.headerclone) .search_trigger_mobile').on("click",function(){
		jQuery('.deep_search_input.search_input').addClass('open');
		setTimeout(function(){ 
			jQuery('.deep_search_input .search_input_value').focus();
		}, 1500);
	});
	jQuery(document).on('click', '.deep_search_input .search_close', function(){
		form.find('.ajax_search_results ul').html("");
		jQuery('.deep_search_input.search_input').removeClass('open');
		jQuery('.deep_search_input.search_input input').blur().val('');
	});
	
		
	if (window.deepOptions.deep_enable_ajax_search == "on"){
					...
					var des_search_timer = setTimeout(function(){
						if (form.find('.search_input_value').val().length > 0){
							...
							search_ajaxing = jQuery.ajax({
								...
								url: ajaxurl,
								data: {
									action : 'call_boldway_search_ajax',
									se: window.deepOptions.searcheverything,
									query: form.serialize().substr(2),
									wpml_lang: window.deepOptions.deep_wpml_current_lang,
									dataType: 'JSON',
									security: jQuery('#deep-theme-search').val()
								},
...
jQuery(window).resize(function(event){
	if (jQuery('html,body').hasClass('deep-push-sidebar-opened') && !window.fromSlidingPanel){
		jQuery('.deep-push-sidebar').add(jQuery('header:not(.headerclone)')).add(jQuery('body > #main')).addClass('none-transition');
		jQuery('html,body').removeClass('deep-push-sidebar-opened');
		jQuery('.deep-push-sidebar').removeClass('opened');
		jQuery('.sliderbar-menu-controller .hamburguer-trigger-menu-icon').removeClass('sidebar-opened');
	} else {
		jQuery('.deep-push-sidebar').add(jQuery('header:not(.headerclone)')).add(jQuery('body > #main')).removeClass('none-transition');
	}
...
jQuery(window).on("load", function(){
		...
		if (jQuery('a.cbp-l-loadMore-link:not(.cbp-l-loadMore-stop)').length){
			jQuery('a.cbp-l-loadMore-link').on("click",function(){
				if (!jQuery(this).hasClass('cbp-l-loadMore-stop')){
					...
					var boldwayInitialCubeItems = jQuery(this).parent().parent().siblings('.cbp').find('img').length;
					var boldwayCheckForNewCubeItems = setInterval(function(){
						if (thisLoadMore.parent().parent().siblings('.cbp').find('img').length > boldwayInitialCubeItems){
							clearInterval(boldwayCheckForNewCubeItems);
							thisLoadMore.parent().parent().siblings('.cbp').find('img:not(.deep_grayscale)').addClass('deep_grayscale');
						}
					}, 200);	
				}
			});
	...
	if (window.deepOptions.deep__portfolio_grayscale_effect == "on") {
	...
	/* grayscales only on thumbnails*/
	if (window.deepOptions.deep__instagram_grayscale_effect == "on") {
		jQuery('.cbp-wrapper img').each(function(){
			jQuery(this).addClass('deep_grayscale');
	...
	if (window.boldwayOrigHash && !window.scrollHappened && window.isWLoaded) {
		var sectionid = window.boldwayOrigHash;
		if (jQuery('section[data-section-title="'+sectionid.substr(1)+'"]').length) {
			sectionid = "#"+jQuery('section[data-section-title="'+sectionid.substr(1)+'"]').attr('id');
		}
		if (jQuery('header:not(.headerclone) .navbar-collapse a[href="'+sectionid+'"]').length && !window.boldwayWaitOnRev && !window.boldwayWaitOnTabs && window.isDLoaded){
			setTimeout(function(){
				jQuery(window).trigger('resize'); 
				if (typeof Waypoint == "function") Waypoint.refreshAll();
		...
		if (window.deepOptions.deep_update_section_titles === "off" && window.location.hash != "" && !jQuery('body').hasClass('page-template-template-side-nav')){
...
jQuery(document).ready(function(){
	...
	if (window.BrowserDetect.isModernIE) setTimeout(function(){ jQuery('header .search_trigger, header .menu-controls, header .deep_dynamic_shopping_bag, .header_social_icons.with-social-icons').css('display','table-cell'); }, 1000);
					...
					if (!$this.children('.sub-arrow').is(':hover')){
						jQuery('html,body').animate({
						  scrollTop: whereTo+45
							...
							easing: "easeInOutQuart",
							complete: function(){
								if (window.deepOptions.deep_update_section_titles == 'on' && target.data('sectionTitle')){
	...
	jQuery(document).trigger('boldwayDLoad');
	
	jQuery('header:not(.headerclone) .menu_style5_bearer > .deep-header-button').appendTo(jQuery('header:not(.headerclone) .menu_style5_bearer > ul'));
		
	if (window.BrowserDetect.browser === "Explorer" && window.BrowserDetect.version == 9){
		/* disable address update */
		window.deepOptions.deep_update_section_titles = 'off';
	...
	if(navigator.userAgent.match(/Trident\/7\./)) { // if IE
		...
		jQuery('body').on("scroll", function (event) {
			...
			boldwayScrollTo(currentScrollPosition - wheelDelta, 500);
		});
	}
	
	jQuery('select:not(.deep-push-sidebar-content select):not(.des-pages select)').selectWoo();
	...
	if (jQuery('#deep_website_load').length){
		QueryLoader2(document.querySelector("body"), {
	        ...
	        minimumTime: 500,
	        onComplete: function(){
		        jQuery('#deep_website_load').fadeOut(1000, function(){
    		        jQuery(this).remove();
    	        });
	        }
		});
	...
	/* icon services new effect from deep */
	...
	if (jQuery('#mc-embedded-subscribe').length){
		jQuery('#mc-embedded-subscribe').on("click",function(e){
			if (!deep_validate_email(jQuery('#mce-EMAIL').val())){
				e.stopPropagation();
				e.preventDefault();
	...
	jQuery('.news-l input[type="email"]').attr('placeholder',window.deepOptions.deep_newsletter_input_text);
	jQuery('header:not(.headerclone) .nav-container, .header_style8_left .navbar-collapse, .header_style9_left .navbar-collapse').on('mouseenter', function(){ correct_deep_mega_menu(); });
		
	jQuery('header .navbar-nav > li:not(.deep_mega_menu) li, header .navbar-nav > li:not(.deep_mega_menu) li').each(function(){
		deep_check_menu_right_frontier(jQuery(this));
	});
	...
	jQuery('.deep_dynamic_shopping_bag').siblings('.search_trigger').addClass('next-to-shopping-bag');
	jQuery('.deep_dynamic_shopping_bag').siblings('.menu-controls').addClass('menu-next-to-shopping-bag');
	...
	jQuery(document).on("click", ".deep_menu_trigger .menu_trigger", function(){
		if (jQuery(".deep_menu_trigger").is(".menu-opened")){
			jQuery(".header_style9_left_container .mobile-opened").removeClass("mobile-opened");
			jQuery(".deep_menu_trigger, .deep_menu_trigger .hamburguer-trigger-menu-icon").removeClass("menu-opened");
		} else {
			jQuery(".deep_menu_trigger, .deep_menu_trigger .hamburguer-trigger-menu-icon").addClass("menu-opened");
		}
	});
	...
	if (window.boldwayOrigHash) {
		...
		if (jQuery('header:not(.headerclone) .navbar-collapse a[href="'+sectionid+'"]').length && jQuery('#home.revslider rs-module').length){
			...
			window.boldwayWaitOnRev = true;
			jQuery(document).scrollTop(0);
			auxrevapi.one("revolution.slide.onloaded", function(){
				window.boldwayWaitOnRev = false;
				if (window.isWLoaded && !window.scrollHappened && !window.boldwayWaitOnTabs){
					setTimeout(function(){ 
						jQuery(window).trigger('resize'); 
						if (typeof Waypoint == "function") Waypoint.refreshAll();
						jQuery('header:not(.headerclone) .navbar-collapse a[href="'+sectionid+'"]').trigger('click'); 
...
jQuery(document).one('boldway_st_ready', function(){
	if (window.isWLoaded && !window.scrollHappened && window.boldwayOrigHash){
		var sectionid = window.boldwayOrigHash;
		if (jQuery('section[data-section-title="'+sectionid.substr(1)+'"]').length) {
			sectionid = "#"+jQuery('section[data-section-title="'+sectionid.substr(1)+'"]').attr('id');
		}
...
function boldwayScrollTo( to, duration ) {
...
function boldway_special_tabs(fromcube){
	...
	if (jQuery('.special_tabs:not(.special_tabs_ready)').length){
		...
		jQuery('.special_tabs:not(.special_tabs_ready)').each(function(e){
			...
			jQuery('.'+elm).find('.tab-selector .label').each(function(){
				...
				if (!jQuery(this).find('.deep_icon_special_tabs').length){
				...
				jQuery(this).on("click",function(){ 
					...
					if (!jQuery(this).hasClass('current')){
						...
						jQuery('.'+elm).find('.tab-selector .label.'+randid).css({'color': window.deepOptions.styleColor });
			...
			jQuery('.animateMinHeight').on('animationend webkitAnimationEnd mozAnimationEnd oAnimationEnd transitionend webkitTransitionEnd mozTransitionEnd oTransitionEnd', function(){
				if (typeof Waypoint === "function") Waypoint.refreshAll();
				jQuery(document).trigger('boldway_st_ready');
				window.boldwayWaitOnTabs = false;
			});
...
function deep_validate_email(email) {
...
function deep_check_menu_right_frontier(el){
...
jQuery(window).on("load", function(){ jQuery('img, a:not(.social-network)').on('mouseenter', function(){ jQuery(this).data('title', jQuery(this).attr('title') ).removeAttr('title'); }).on('mouseleave',function(){ jQuery(this).attr('title', jQuery(this).data('title') ); }); jQuery('form.deep_search_input').removeClass('not-ready'); });