		...
		jQuery( '.vc_progress_bar' ).waypoint( function () {
			jQuery( this.element ).find( '.vc_single_bar.notinview' ).each( function ( index ) {
				...
				var t = jQuery( this ),
					bar = t.find( '.vc_bar' ),
					val = bar.data( 'value' ),
					units = t.find('.vc_label_units').data('units'),
					maxleft = val > 100 ? 100 : val,
					right = t.find('.pointerval').width()+2,
					pointervalwidth = Math.ceil(t.find('.pointerval').width())+1+'px';
				
				t.find(".pointerval").css('width', '0%');
				t.removeClass('notinview').find('.vc_label_units').html('0'+units).css({'float':'right'});
				...
				setTimeout( function () {
					...
					bar.css("width",0).animate( { "width": maxleft + '%' },{
						...
						step: function(a, p, c) {
							...
							t.find(".pointerval").css( { "width": Math.round(a) + '%' });
						},
						complete: function(){
